...
class MaskedSpectralLoss(nn.Module):
    ...
    def __init__(self, scaler: nn.Module) -> None:
        super().__init__()
        self.scaler = scaler
        self.register_buffer('log_conversion', torch.log(torch.tensor(10.0)) / 10)
    ...
    def forward(self, pred_norm: torch.Tensor, target_norm: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        numerator = torch.sqrt(torch.sum(diff * diff) + 1e-6)
        denominator = torch.sqrt(torch.sum(target_lin * target_lin) + 1e-6)
